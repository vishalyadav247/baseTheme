...
(function () {
  ...
  class Slider {
    constructor(root) {
      this.root = root;
      this.track = qs(root, ".product-media-track");
      if (!this.track) return;

      // Main
      this.slides = qsa(root, ".product-media-slide");
      this.prevBtn = qs(root, ".product-media-prev");
      this.nextBtn = qs(root, ".product-media-next");
      this.dots = qsa(root, ".product-media-dot");
      this.counterCurrent = qs(root, ".product-media-current");

      // Thumbs
      this.thumbWrap = qs(root, ".product-media-thumbs-track");
      this.thumbBtns = qsa(root, ".product-media-thumb");
      this.thumbPrev = qs(root, ".product-media-thumbs-prev");
      this.thumbNext = qs(root, ".product-media-thumbs-next");

      // Lightbox
      this.lb = qs(root, ".product-media-lightbox");
      this.lbImg = qs(root, ".product-media-lightbox-img");
      this.lbPrev = qs(root, ".product-media-lightbox-prev");
      this.lbNext = qs(root, ".product-media-lightbox-next");
      this.lbClose = qs(root, ".product-media-lightbox-close");
      this.lbCur = qs(root, ".product-media-lightbox-current");
      this.lbTot = qs(root, ".product-media-lightbox-total");
      this.lbBackdrop = qs(root, ".product-media-lightbox-backdrop");
      ...
      // Register this instance by section id
      const secId = (this.root.id || "").replace("product-media-gallery-", "");
      if (secId) window.ProductMediaGallery.registry[secId] = this;

      // Build map: mediaId → slide index
      this.byMediaId = new Map();
      this.slides.forEach((s, i) => {
        const mid = s.getAttribute("data-media-id");
        if (mid) this.byMediaId.set(String(mid), i);
      });
    ...
    thumbsHorizontal() {
      return (
        this.thumbWrap &&
        getComputedStyle(this.thumbWrap).display.includes("flex")
      );
    }
    pageStep() {
    ...
    snap() {
      this.track.style.transition = "transform 280ms ease";
    ...
    prev() {
      ...
      else if (this.loop && this.total) this.index = this.total - 1;
      this.update(true);
    }
    next() {
      if (this.index < this.total - 1) this.index++;
      else if (this.loop && this.total) this.index = 0;
      this.update(true);
    }

    goToMediaId(mediaId) {
      if (!mediaId) return;
      const idx = this.byMediaId.get(String(mediaId));
      if (typeof idx === "number") this.goTo(idx);
    }
    ...
    ensureActiveThumbInView() {
      ...
      if (this.thumbsHorizontal()) {
        if (itm.left < wrap.left)
          this.thumbWrap.scrollBy({
            left: itm.left - wrap.left - 6,
            behavior: "smooth",
          });
        if (itm.right > wrap.right)
          this.thumbWrap.scrollBy({
            left: itm.right - wrap.right + 6,
            behavior: "smooth",
          });
      } else {
        if (itm.top < wrap.top)
          this.thumbWrap.scrollBy({
            top: itm.top - wrap.top - 6,
            behavior: "smooth",
          });
        if (itm.bottom > wrap.bottom)
          this.thumbWrap.scrollBy({
            top: itm.bottom - wrap.bottom + 6,
            behavior: "smooth",
          });
      }
    }
    ...
    bind() {
      ...
      if (this.thumbWrap) {
        ...
        const maxLeft = () =>
          this.thumbWrap.scrollWidth - this.thumbWrap.clientWidth;
        const maxTop = () =>
          this.thumbWrap.scrollHeight - this.thumbWrap.clientHeight;
        ...
        const animate = () => {
          ...
          if (horiz) {
            ...
            if (Math.abs(diff) < 0.5) {
              this.thumbWrap.scrollLeft = targetX;
              animating = false;
              return;
            }
            ...
            if (Math.abs(diff) < 0.5) {
              this.thumbWrap.scrollTop = targetY;
              animating = false;
              return;
            }
        ...
        this.thumbWrap.addEventListener(
          "wheel",
          (e) => {
            e.preventDefault();
            ...
            if (e.deltaMode === 1) raw *= 16; // lines → px
              ...
              raw *= this.thumbsHorizontal()
                ? this.thumbWrap.clientWidth
                : this.thumbWrap.clientHeight;

            const capped = Math.sign(raw) * Math.min(Math.abs(raw), WHEEL_CAP);
            if (this.thumbsHorizontal()) {
              targetX = Math.max(
                0,
                Math.min(targetX + capped * WHEEL_SCALE, maxLeft())
              );
            } else {
              targetY = Math.max(
                0,
                Math.min(targetY + capped * WHEEL_SCALE, maxTop())
              );
            }
            if (!animating) {
              animating = true;
        ...
        this.thumbWrap.addEventListener(
          ...
          (e) => {
            ...
            if (horiz) {
              ...
              if (
                !this._thumbWasDragging &&
                Math.abs(dx) >= this._thumbDragThreshold
              )
                this._thumbWasDragging = true;
              ...
              if (
                !this._thumbWasDragging &&
                Math.abs(dy) >= this._thumbDragThreshold
              )
                this._thumbWasDragging = true;
        ...
        const jump = (dir) => {
          ...
          if (this.thumbsHorizontal()) {
            targetX = Math.max(
              0,
              Math.min(this.thumbWrap.scrollLeft + dir * step, maxLeft())
            );
          } else {
            targetY = Math.max(
              0,
              Math.min(this.thumbWrap.scrollTop + dir * step, maxTop())
            );
          }
          if (!animating) {
            animating = true;
        ...
        this.thumbPrev?.addEventListener("click", (e) => {
          e.preventDefault();
          jump(-1);
        });
        this.thumbNext?.addEventListener("click", (e) => {
          e.preventDefault();
          jump(1);
        });
      }
      ...
      qsa(this.root, ".product-media-open-lightbox").forEach((btn) => {
        btn.addEventListener("click", (e) => {
          if (this.didDrag) return;
          const slide = btn.closest(".product-media-slide");
    ...
    openLightbox(i) {
      ...
      this.renderLightbox();
      document.documentElement.classList.add("product-media-no-scroll");
    }
    closeLightbox() {
      if (!this.lb) return;
      ...
      document.documentElement.classList.remove("product-media-no-scroll");
    }
    lightboxPrev() {
      if (this.lbIndex > 0) this.lbIndex--;
      else if (this.loop && this.total) this.lbIndex = this.total - 1;
      this.renderLightbox();
    }
    lightboxNext() {
      if (this.lbIndex < this.total - 1) this.lbIndex++;
      else if (this.loop && this.total) this.lbIndex = 0;
      this.renderLightbox();
    }
    renderLightbox() {
